...
internal static class Program {
    ...
    private static void Main(string[] args) {
        ...
        if (multiThread) {
            var random = new ThreadLocal<Random>(() => new Random(12345));
            Parallel.For(0, w * h, i => c[i] = sample(random.Value, i));
        }
        else {
        ...
        var sw = File.CreateText("smallptCS.ppm"); // Write image to PPM file.
        sw.Write("P3\n{0} {1}\n{2}\n", w, h, 255);
        for (int i = 0; i < w * h; i++) {
